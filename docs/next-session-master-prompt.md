...
- physical namespace relocation의 import inventory와 foundation/identity slice는 완료됐으며 다음 slice는 Dashboard다.
...
- external capability가 blocked이면 remaining physical package relocation slice를 선택한다. Foundation/identity를 반복하지 말고 Dashboard부터 진행한다. PostgreSQL repository graph는 이미 구현돼 있으므로 중복 작성하지 않는다.
