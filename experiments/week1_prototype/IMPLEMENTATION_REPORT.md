...
| 평가 요소 | 가중치 | FastAPI 점수 | FastAPI 판단 | Flask 점수 | Flask 판단 |
|---|---:|---:|---|---:|---|
| 개발 완성도와 구현 생산성 | 25% | 5/5 | typed router·Pydantic·의존성 주입으로 큰 API를 일관되게 구성 | 3/5 | 시작은 단순하지만 큰 API의 구조와 확장 도구를 별도로 조합해야 함 |
| API 계약과 문서 자동화 | 25% | 5/5 | OpenAPI 172개와 전체 성공 응답 계약 자동 생성 | 2/5 | 확장 도구로 구현 가능하지만 bare Flask 기본 구성에는 없음 |
| 요청·응답 검증과 오류 안전성 | 25% | 5/5 | 요청 검증 147개, JSON 응답 검증 168개, 미처리 5xx 0건 | 2/5 | 라우팅은 검증했지만 전체 업무 응답 검증은 미구현 |
| 최소 API 경량성과 단순 응답 속도 | 25% | 2/5 | `/health` p50 0.7603ms | 5/5 | `/health` p50 0.0766ms |
| **가중 합계** | **100%** | **85점** |  | **60점** |  |
