      ...
      <section className="ontology-workbench-grid">
        <aside className="ontology-object-rail">
          ...
          <div className="ontology-type-list">
            {objectTypes.map((item) => <button type="button" key={item.id} className={selectedType === item.id ? "active" : ""} onClick={() => setSelectedType(item.id)}><span><Boxes size={12} /></span><div><strong>{item.display_name}</strong><small>{item.domain_pack} · {item.properties.length} properties</small></div></button>)}
          </div>
          ...
          <div className="ontology-primary-view">
            ...
            {view === "exploration" ? (
              <div className="ontology-exploration-view">
                {selectedObject ? <><article className="ontology-exploration-root"><span><Boxes size={18} /></span><div><small>{selectedObject.object_type}</small><strong>{objectIdentity(selectedObject)}</strong><code>{selectedObject.id}</code></div></article><div className="ontology-exploration-links">{traversalLoading ? <LoadingState title="Loading links" /> : traversal?.edges.map((edge) => { const relatedId = edge.source_object_id === selectedObject.id ? edge.target_object_id : edge.source_object_id; const related = traversal.nodes.find((item) => item.id === relatedId); return <article key={edge.id}><StatusPill intent="primary">{edge.link_type}</StatusPill><GitBranch size={14} /><div><strong>{related ? objectIdentity(related) : relatedId}</strong><small>{edge.source_object_id} → {edge.target_object_id}</small></div></article>; })}</div></> : <ErrorState title="Select an object" detail="Choose an object from Table view to explore governed links." />}
              </div>
            ) : null}
            {view === "graph" ? (
        ...
        <ObjectViewInspector object={selectedObject} definition={selectedDefinition} traversal={traversal} traversalLoading={traversalLoading} actions={objectActions} status={status} permissions={permissions} actionBusyId={actionBusyId} actionNotice={actionNotice} onInvokeAction={(action) => void invokeAction(action)} />
      </section>
