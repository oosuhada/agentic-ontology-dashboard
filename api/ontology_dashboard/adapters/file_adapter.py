...
class FileAdapter:
    ...
    def ingest(self, manifest: DatasetManifest) -> IngestionResult:
        ...
        if manifest.schema_.format != "csv":
            raise ValueError("the current File Adapter runtime supports CSV manifests")
        ...
        try:
            with path.open(
                ...
                alias_mapping, missing = self._resolve_headers(reader.fieldnames, manifest)
                ...
                for row_number, raw_record in enumerate(reader, start=2):
                    ...
                    record = self._apply_aliases(
                        {key: value or "" for key, value in raw_record.items() if key is not None},
                        alias_mapping,
                    )
                    quality_issue = next(
                        (
