...
| 호범 | 팀원3 | LLM을 활용한 백엔드 리포트 자동 생성 | 리포트 생성 API, Prompt·Schema, 근거 기반 생성, 검증 및 예시 결과 |
| 성민 | 팀원4 | 예측 결과 조회 API와 데이터·예측 파이프라인 및 재현성 | 조회 API, 파이프라인, Manifest, Checksum, 재현성 검증 결과 |
...
## 7. 성민 — 팀원4 — 예측 결과 조회 API + 데이터·예측 파이프라인 및 재현성

### 7.1 역할 목적

성민은 데이터·예측 파이프라인과 예측 결과 조회 API를 통합해 담당한다. Canonical V3.1에서 예측 결과를 재현 가능하게 생성하고, 우수의 화면과 호범의 LLM 리포트가 조회할 수 있는 API로 제공한다.
...
## 8. 호범 — 팀원3 — LLM을 활용한 백엔드 리포트 자동 생성
