...
- 연관 문서:
  - [Operations 범위 및 4개 화면 명세서](./operations-scope-and-screen-specification.md)
  - [Operations API 명세서](./operations-api-specification.md)
  - [Operations 데이터 계약서](./operations-data-contract.md)
...
1. 요구사항 정의서 작성
2. Operations를 어떻게 구현할지 설명하는 설계 계획서 작성
3. 실제로 확인할 수 있는 Operations 화면 공유
...
## 2. 멘토링에서 확정된 Operations 기준

### 2.1 데이터

- Canonical V3.1을 Operations의 단일 기준 데이터로 사용한다.
- 새로운 데이터셋을 다시 찾거나 교체하는 작업은 이번 범위에 포함하지 않는다.

### 2.2 화면

Operations 화면은 다음 네 개로 제한한다.
...
`Analysis` 화면은 이번 Operations에서 제외한다.
...
시스템 관리자, 모델 검증 담당자와 품질 감사 담당자 등 전체 역할 확장은 Operations 이후 단계로 미룬다.
...
- Overview·Objects·Operations·Executive Report View 네 화면을 Operations 핵심 범위로 유지한다.
...
|---|---|---|---|
| 우수 | 팀원1 | Operations 프론트엔드와 화면 연결 | 4개 화면, API 연결, 화면 링크, 화면 캡처, 사용자 흐름 |
| 광우 | 팀원2 | 요구사항·리포트·스키마·기능·API·Operations 설계 문서화 | 요구사항 정의서, 리포트 정의서, 스키마 정의서, 기능 명세서, API 명세서, Operations 설계 명세서 |
...
## 5. 우수 — 팀원1 — Operations 프론트엔드와 화면 연결

### 5.1 역할 목적

우수는 다음 주에 공유할 Operations의 프론트엔드 담당자다. 화면을 새로 크게 확장하는 것보다 이미 구현된 V1·V2 화면에서 확정된 네 화면을 추려 하나의 업무 흐름으로 연결하고, 성민의 API와 호범의 리포트 생성 기능을 실제 UI에 연결하는 것이 핵심이다.

### 5.2 필수 작업

#### A. 네 개 Operations 화면 정리

| Operations 화면 | 기준 구현 | 핵심 사용자 |
|---|---|---|
...
- `Analysis` 메뉴를 Operations 동선에서 제외
- 네 화면 사이 이동 경로 고정
...
쓰기 기능이 아직 준비되지 않았다면 읽기 전용 화면으로 범위를 제한하고, 버튼은 `Operations 데모 Action`인지 실제 저장 Action인지 구분한다.
...
## 6. 광우 — 팀원2 — 요구사항·리포트·스키마·기능·API·Operations 설계 문서화

### 6.1 역할 목적

광우는 Operations의 문서 계약을 고정하는 담당자다. 단순 회의록 작성이 아니라, 우수·성민·호범이 서로 다른 해석으로 구현하지 않도록 사용자 요구, 화면, 데이터, API와 리포트 계약을 하나의 문서 체계로 연결해야 한다.
...
- Operations 설계 명세서 — `week2-operations-design-specification.md`
...
```text
docs/10-product/mentoring-operations-2026-08/
...
└── week2-operations-design-specification.md
```
...
| 기능 명세서 | 광우 | 우수 |
| API 명세서 | 광우 | 성민·호범 |
| Operations 설계 명세서 | 광우 | 전체 |
| 파이프라인·재현성 | 성민 | 광우 |
| 예측 조회 API | 성민 | 우수·호범 |
| Operations 프론트엔드 | 우수 | 성민·호범 |
| LLM 리포트 백엔드 | 호범 | 광우·성민·우수 |
| Operations 화면 공유 | 우수 | 전체 |
