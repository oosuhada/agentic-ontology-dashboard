"""Current MVP service composition and request dependencies."""
...
def get_project_service() -> ProjectService:
    ensure_database_migrations()
    target = database_target()
    repository = PostgreSQLProjectRepository(target) if is_postgresql(target) else ProjectRepository(target)
...
def get_ontology_service(
    ...
    if not project_id:
        raise AuthError(409, "active_project_required", "Action을 기록하기 전에 Project를 활성화해야 합니다.")
...
def current_principal(
    ...
    return _ontology_principal(request, identity)
