# Ontology Dashboard MVP 데이터 계약서

- 상태: 멘토링 합의 기반 MVP 공통 데이터 계약
...
이 문서는 Canonical V3.1의 데이터와 모델 결과를 네 개 MVP 화면에서 동일한 의미로 사용하기 위한 공통 계약을 정의한다.
...
MVP의 기본 Dataset Version은 release-ready Canonical V3.1이다.
...
| `equipment` | MvpEquipment | 아니오 | 설비 요약 |
...
MVP는 절대적인 단일 최적 threshold를 주장하지 않는다.
...
MVP 데이터에는 실제 고객 개인정보를 포함하지 않는다.
...
| ID | Gap | 우선순위 | 완료 기준 |
|---|---|---:|---|
| `MVP-DATA-GAP-01` | Assignment 공통 모델 미고정 | P1 | schema·저장·조회 구현 |
| `MVP-DATA-GAP-02` | Activity type enum 미표준 | P1 | 공통 enum·adapter 구현 |
| `MVP-DATA-GAP-03` | Confidence 문자열 비정규 | P1 | 4개 enum 정규화 |
| `MVP-DATA-GAP-04` | Report와 Evidence 불일치 검증 부족 | P2 | 경고·테스트 추가 |
| `MVP-DATA-GAP-05` | Threshold recommendation payload 없음 | P2 | 가정 포함 schema 추가 |
