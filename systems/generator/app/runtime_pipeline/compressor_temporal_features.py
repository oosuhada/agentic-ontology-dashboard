"""Runtime implementation of the Canonical V3.1 compressor feature contract."""
...
def derive_compressor_temporal_feature_rows(
    frame: pd.DataFrame,
    *,
    ...
    """Return one inference-ready row per compressor using its latest history."""
    engineering = feature_schema.get("feature_engineering") or {}
    if engineering.get("kind") != SUPPORTED_KIND:
        raise ValueError(
            "unsupported compressor feature engineering kind: "
            f"{engineering.get('kind')!r}"
        )
    ...
    if not expected_features:
        raise ValueError("compressor feature schema has no declared features")
    ...
    if total_required != 36:
        raise ValueError(
            "compressor-temporal-v2 requires exactly 35 prior observations plus "
    ...
    for raw_asset_id, group in frame.groupby(id_column, sort=False):
        ...
        if not isinstance(asset_baseline, dict):
            raise ValueError(
                f"compressor asset {asset_id} has no calibrated baseline in Model Artifact"
            )

        recent_frame = group.iloc[-total_required:].copy()
        timestamps = pd.to_datetime(
            recent_frame[time_column], utc=True, errors="raise"
        )
        if not timestamps.is_monotonic_increasing or timestamps.duplicated().any():
            raise ValueError(
                "compressor runtime history timestamps must be strictly increasing"
            )
        cadence = timestamps.diff().dropna().dt.total_seconds().div(60.0)
        if not cadence.empty and float(
            (cadence - expected_cadence).abs().max()
        ) > max(1.0, expected_cadence * 0.25):
            raise ValueError(
                "compressor runtime history cadence is incompatible with "
                f"{expected_cadence:g}-minute contract"
            )
        ...
        for sensor in sensors:
            if sensor not in recent_frame:
                raise ValueError(
                    f"compressor runtime observation missing sensor: {sensor}"
                )
            ...
            if not np.isfinite(mean) or not np.isfinite(std) or std <= 0:
                raise ValueError(
                    f"compressor baseline for {asset_id}/{sensor} is invalid"
                )
            values = pd.to_numeric(
                recent_frame[sensor], errors="raise"
            ).astype(float)
        ...
        for sensor in sensors:
            ...
            calculated[f"{sensor}_6h_change"] = float(
                (current - float(series.iloc[-36])) / 6.0
            )
            calculated[f"{sensor}_1h_change"] = float(
                current - float(series.iloc[-7])
            )
        ...
        if missing:
            raise ValueError(
                "compressor runtime feature implementation missing schema features: "
                f"{missing}"
            )
        ...
        if not np.isfinite(np.asarray(feature_row, dtype=np.float64)).all():
            raise ValueError("compressor runtime features contain NaN or Inf")
        rows.append(feature_row)
        metadata.append(
            (asset_id, timestamps.iloc[-1].strftime("%Y-%m-%dT%H:%M:%SZ"))
        )
