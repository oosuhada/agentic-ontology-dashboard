...
class LayoutPlanner:
    def __init__(self, project_root: str | Path, provider: LLMProvider | None = None) -> None:
        ...
        self.schema = json.loads((self.root / "schemas" / "ui-block.schema.json").read_text(encoding="utf-8"))

    def _ordered_types(self, evidence: dict[str, Any], role: Role, intent: Intent) -> list[str]:
        if evidence["status"] == "data_quality_hold":
            return (
                ["DataQualityWarning", "StatusSummary", "ManagerDecisionCard", "ModelDetails", "ConversationThread"]
                if role == "manager"
                else ["DataQualityWarning", "EvidenceTable", "EngineerChecklist", "ModelDetails", "ConversationThread"]
            )
        if evidence["confidence"] == "low":
            return (
                ["DataQualityWarning", "StatusSummary", "RiskKpi", "ManagerDecisionCard", "PriorityList", "RecommendedActions", "ModelDetails", "ConversationThread"]
                if role == "manager"
                else ["DataQualityWarning", "SensorLineChart", "EvidenceTable", "EngineerChecklist", "FactorContribution", "RecommendedActions", "ModelDetails", "ConversationThread"]
            )
        ...
        if role == "engineer" and intent == "overview":
            ...
            promoted = (
                ["StatusSummary"]
                if scenario_id == "GS-001"
                else ["FactorContribution"]
                if scenario_id == "GS-005"
    ...
    def deterministic(self, evidence: dict[str, Any], role: Role, intent: Intent, *, mode: str) -> UILayout:
        block_types = self._ordered_types(evidence, role, intent)
        blocks = []
        for index, block_type in enumerate(block_types, start=1):
            title, fields = BLOCK_REGISTRY[block_type]
            if block_type == "DataQualityWarning" and evidence["confidence"] == "low" and not evidence["data_quality_warnings"]:
                title = "저신뢰 결과 경고"
    ...
    def validate(self, layout: UILayout, evidence: dict[str, Any]) -> None:
        ...
        errors = sorted(Draft202012Validator(self.schema).iter_errors(payload), key=lambda item: list(item.absolute_path))
        if errors:
    ...
    def plan(
        ...
        *,
        ...
        if not use_llm:
            return self.deterministic(evidence, role, intent, mode="deterministic"), {"fallback": False, "provider": "none"}
        ...
        try:
            ...
            payload = self.provider.generate_json(
                prompt,
                {"role": role, "intent": intent, "evidence": evidence, "report": report.model_dump(mode="json")},
            )
            payload["mode"] = "llm"
