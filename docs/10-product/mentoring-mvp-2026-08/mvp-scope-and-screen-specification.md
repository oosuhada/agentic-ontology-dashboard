# Ontology Dashboard MVP 범위 및 4개 화면 명세서
...
이 문서는 현재까지 확장된 전체 제품 기능 중 발표와 MVP 검증에 필요한 최소 범위를 다시 고정한다. 목표는 많은 메뉴를 구현하는 것이 아니라, Canonical V3.1의 설비 위험 정보를 이용해 생산 관리자와 현장 담당자가 하나의 사건을 확인하고 조치하며, 결과를 임원 보고서로 전달하는 흐름을 네 화면 안에서 완결하는 것이다.
...
이 문서가 승인된 이후 MVP 관련 신규 구현은 아래 네 화면의 완성도와 연결성을 우선한다.
...
| Analysis | MVP에서 제외 | 메뉴 숨김 또는 비활성 처리, 발표 동선과 완료 조건에서 제거 |
...
| 2주차 | 요구사항 정의서, MVP 설계안, 화면 공유 | 본 문서를 요구사항 기준선으로 사용하고 4개 화면을 공유 가능 상태로 완성 |

## 3. MVP 한 문장 정의

> Canonical V3.1 기반 설비 위험 정보를 생산 관리자가 우선순위로 확인하고, 현장 담당자가 대상 설비와 근거를 검토해 점검·조치 상태를 기록하며, 그 결과를 임원 보고서로 전달하는 제조 예지보전 의사결정 지원 MVP.
...
MVP에서는 아래 기존 역할을 하나의 사용자 그룹으로 묶는다.
...
### 4.3 이번 MVP에서 사용자로 다루지 않는 역할

아래 역할은 계정과 기존 화면을 삭제하지 않지만 MVP 시나리오·화면 요구사항·발표 동선의 핵심 사용자에서 제외한다.
...
관리자, 감사, 모델 운영 기능은 시스템 유지용 기존 기능으로 남기되 이번 MVP 완료 판단에는 포함하지 않는다.
...
- 기존 구성 요소 중 `Analysis 열기` 동선은 제거하거나 MVP에서 비활성화한다.
...
모든 필터를 한 번에 구현하기 어렵다면 2주차 MVP 필수 필터는 `검색`, `라인`, `위험 상태`, `담당자` 네 가지로 제한한다.
...
미탐과 오탐의 상대 비용은 업종, 설비 중요도, 생산 계획과 고객 정책에 따라 달라진다. 따라서 MVP는 하나의 임계값을 모든 고객에게 적용되는 최적값으로 주장하지 않는다.

### 12.2 MVP 제공 수준

MVP는 아래 입력을 바탕으로 권장 임계값 또는 권장 범위를 제시한다.
...
- 별도 모델링 화면은 MVP 범위에 추가하지 않는다.

## 13. MVP 제외 범위

다음은 구현되어 있거나 향후 필요하더라도 이번 MVP 완료 조건에서 제외한다.
...
| ID | 요구사항 | 우선순위 |
|---|---|---|
| MVP-FR-001 | Canonical V3.1을 네 화면의 공통 데이터 기준으로 사용한다. | Must |
| MVP-FR-002 | Overview에서 위험 설비 우선순위와 생산 영향을 확인한다. | Must |
| MVP-FR-003 | Overview 선택 상태를 Objects와 Operations로 전달한다. | Must |
| MVP-FR-004 | Objects에서 설비 검색·필터·상세 확인을 제공한다. | Must |
| MVP-FR-005 | Operations에서 점검 요청과 상태 변경을 기록한다. | Must |
| MVP-FR-006 | 생산 관리자와 현장 담당자가 동일 Event 상태를 공유한다. | Must |
| MVP-FR-007 | 임원 보고서가 위험·영향·대응·근거를 요약한다. | Must |
| MVP-FR-008 | 보고서와 운영 화면의 수치가 동일한 계약에서 계산된다. | Must |
| MVP-FR-009 | 저신뢰·데이터 오류를 고장 확정으로 표현하지 않는다. | Must |
| MVP-FR-010 | V2 Analysis를 MVP 메뉴·동선에서 제외한다. | Must |
| MVP-FR-011 | 미탐·오탐 비용 가정에 따른 임계값 권장 정보를 제공한다. | Should |
| MVP-FR-012 | Executive Report를 A4 또는 PDF로 출력할 수 있다. | Should |
| MVP-FR-013 | 라인·상태·담당자 필터를 제공한다. | Should |
| MVP-FR-014 | 현장 측정값과 체크리스트를 상세 기록한다. | Could |
...
아래 조건을 모두 충족하면 2주차 MVP 화면 공유가 가능하다고 판단한다.
...
- V3·V4 개발 결과는 삭제하지 않지만 이번 MVP 동선에 강제로 포함하지 않는다.
